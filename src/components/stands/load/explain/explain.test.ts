...
import { LOAD_SCENARIOS, retryStorm, shedding } from '../engine/scenarios.ts'
import { EVENT_IMPORTANCE } from '../engine/types.ts'
import type { LoadScenario } from '../engine/types.ts'
...
const ALL: LoadScenario[] = [
  ...LOAD_SCENARIOS,
  { ...retryStorm, id: 'cancel', config: { ...retryStorm.config, cancelOnTimeout: true, backoff: true } },
  { ...retryStorm, id: 'no-retries', config: { ...retryStorm.config, retries: 0 } },
  { ...shedding, id: 'limited', config: { ...shedding.config, queueLimit: 8 } },
]
