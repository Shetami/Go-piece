...
import type { Frame, LoadEvent, LoadEventType } from '../engine/types.ts'
 ...
 * события сервиса под нагрузкой: свой список важности и свой разбор.
 */

const markClass = (t: LoadEventType) => `mark mark-ld-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: LoadEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: LoadEvent[]
  selected: LoadEvent | null
  onSelect: (e: LoadEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-ld-${e.type.split('.')[0]}`} />
