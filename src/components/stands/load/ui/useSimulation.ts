...
import type { LoadEvent, LoadEventType, LoadScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: LoadScenario, opts: { autoPause: boolean }): Playback {
  ...
  const [pausedOn, setPausedOn] = useState<LoadEvent | null>(null)

  const seen = useRef(new Set<LoadEventType>())
