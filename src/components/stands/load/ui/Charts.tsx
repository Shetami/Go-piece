...
import type { LoadLog } from '../engine/types.ts'
 ...
 * ось не прыгала. Цвет закреплён за сущностью: синий — работа и обычная
 * задержка, оранжевый — ожидание и хвост, бирюзовый — входящий поток,
 * красный — работа впустую.
 */
...
export function chartData(log: LoadLog): ChartData {
  ...
  return {
    busy: log.series.map((s) => s.busy),
    queue: log.series.map((s) => s.queue),
    p50,
    p99,
    arrived: sum((i) => log.series[i]!.arrived),
    ok: sum((i) => log.series[i]!.ok),
    wasted: sum((i) => log.series[i]!.wasted),
    anyWasted: log.series.some((s) => s.wasted > 0),
  }
}
