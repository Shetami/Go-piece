...
import type { ReplEvent, ReplEventType, ReplScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: ReplScenario, opts: { autoPause: boolean }): Playback {
  const sim = useMemo(() => new Simulation(scenario).runToEnd(Math.min(scenario.stopAfter ?? 200, HARD_CAP)), [scenario])
  ...
  const [pausedOn, setPausedOn] = useState<ReplEvent | null>(null)

  const seen = useRef(new Set<ReplEventType>())
