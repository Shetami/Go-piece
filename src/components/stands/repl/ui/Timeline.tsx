...
import type { Frame, ReplEvent, ReplEventType } from '../engine/types.ts'
 ...
 * события репликации: свой список важности и свой разбор.
 */

const markClass = (t: ReplEventType) => `mark mark-rp-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: ReplEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: ReplEvent[]
  selected: ReplEvent | null
  onSelect: (e: ReplEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-rp-${e.type.split('.')[0]}`} />
    ...
    <div className="detail" role="region" aria-label="Разбор события">
      ...
      {x.pg && (
        <p className="detail-runtime">
          <span>В PostgreSQL:</span> <code>{x.pg}</code>
        </p>
      )}
      {x.model && (
