import type { Fault, LbConfig, LbScenario, Phase } from '../engine/types.ts'
import { resolveConfig } from '../engine/world.ts'
import { lbScenarioById } from '../engine/scenarios.ts'

/**
 * Сценарий ↔ строка для адреса. Хранится только то, что отличается от пресета:
 * `s=black-hole&algo=p2c`. Фазы нагрузки едут в `p` как `40:2,60:1`,
...
const CONFIG_KEYS = Object.keys(resolveConfig(undefined)) as (keyof LbConfig)[]
...
export function initialState(base: LbScenario): StandState {
  return { base: base.id, config: resolveConfig(base.config), phases: null, faults: null }
}

export function toScenario(st: StandState): LbScenario {
  const base = lbScenarioById(st.base)
  if (!base) throw new Error(`нет сценария ${st.base}`)
  // Сбои на репликах, которых нет в начале прогона, выпадают.
  const faults = (st.faults ?? base.faults).filter((f) => f.replica < st.config.replicas)
  return { ...base, config: st.config, phases: st.phases ?? base.phases, faults }
}
...
export function decodeState(query: string, fallback: LbScenario): StandState {
  const q = new URLSearchParams(query.replace(/^[#?]/, ''))
  const base = lbScenarioById(q.get('s') ?? '') ?? fallback
  ...
  st.config = cfg as unknown as LbConfig
  const p = q.get('p')
  if (p !== null) {
    st.phases = p
      .split(',')
