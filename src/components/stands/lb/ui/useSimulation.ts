...
import type { LbEvent, LbEventType, LbScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: LbScenario, opts: { autoPause: boolean }): Playback {
  ...
  const [pausedOn, setPausedOn] = useState<LbEvent | null>(null)

  const seen = useRef(new Set<LbEventType>())
