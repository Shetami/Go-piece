...
import type { Frame, LbEvent, LbEventType } from '../engine/types.ts'
 ...
 * события балансировщика и реплик: свой список важности и свой разбор.
 */

const markClass = (t: LbEventType) => `mark mark-lb-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: LbEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: LbEvent[]
  selected: LbEvent | null
  onSelect: (e: LbEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-lb-${e.type.split('.')[0]}`} />
