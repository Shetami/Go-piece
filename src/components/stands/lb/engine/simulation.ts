import type { Frame, LbEvent, LbLog, LbScenario, LbWorld } from './types.ts'
 ...
 * Снимок мира на каждом тике маленький — реплики с очередями и счётчики, — поэтому
...
export class Simulation {
  readonly scenario: LbScenario
  readonly log: LbLog

  private ctx: Ctx
  private frames: Frame[] = []

  constructor(scenario: LbScenario) {
    this.scenario = scenario
    const world = createWorld(scenario)
    this.log = { series: [emptySample(world.x, world.replicas.length)], done: [] }
  ...
  countEvents(type: LbEvent['type']): number {
    return this.events.filter((e) => e.type === type).length
  }

  firstEvent(type: LbEvent['type']): LbEvent | undefined {
