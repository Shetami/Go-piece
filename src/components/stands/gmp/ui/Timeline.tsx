...
import type { EventType, SimEvent, Snapshot } from '../engine/types.ts'
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: EventType }[] = []
      ...
      {shown.map(({ e, key }) => {
        ...
        return (
          <li key={key}>
            <button
              type="button"
              className={`feed-item imp-${EVENT_IMPORTANCE[e.type]} ${isSel ? 'is-sel' : ''}`}
              onClick={() => onSelect(e)}
            >
