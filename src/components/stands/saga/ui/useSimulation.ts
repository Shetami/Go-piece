...
import type { SagaEvent, SagaEventType, SagaScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: SagaScenario, opts: { autoPause: boolean }): Playback {
  ...
  const [pausedOn, setPausedOn] = useState<SagaEvent | null>(null)

  const seen = useRef(new Set<SagaEventType>())
