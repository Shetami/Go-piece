...
import type { Frame, SagaEvent, SagaEventType } from '../engine/types.ts'
 ...
 * события заказов, брокера и потребителей: свой список важности и свой разбор.
 */

const markClass = (t: SagaEventType) => `mark mark-sg-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: SagaEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: SagaEvent[]
  selected: SagaEvent | null
  onSelect: (e: SagaEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-sg-${e.type.split('.')[0]}`} />
