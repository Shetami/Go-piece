...
import type { SagaLog } from '../engine/types.ts'
 ...
 * ось не прыгала. Цвет закреплён за сущностью: синий — доставленные заказы,
 * оранжевый — заказы в пути между сервисами, бирюзовый — события в брокере
 * и outbox, красный — отменённые и застрявшие.
 */
...
export function chartData(log: SagaLog): ChartData {
  ...
  return {
    created: sum((i) => log.series[i]!.created),
    shipped: sum((i) => log.series[i]!.shipped),
    cancelled: sum((i) => log.series[i]!.cancelled),
    anyCancelled: log.series.some((s) => s.cancelled > 0),
    inFlight: log.series.map((s) => s.inFlight),
    inBroker: log.series.map((s) => s.inBroker),
    outbox: log.series.map((s) => s.outbox),
    anyOutbox: log.series.some((s) => s.outbox > 0),
  }
}
