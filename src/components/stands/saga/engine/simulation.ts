import type { Frame, SagaEvent, SagaLog, SagaScenario, SagaWorld } from './types.ts'
 ...
 * Снимок мира на каждом тике маленький — заказы, брокер и счётчики, — поэтому
...
export class Simulation {
  readonly scenario: SagaScenario
  readonly log: SagaLog

  private ctx: Ctx
  private frames: Frame[] = []

  constructor(scenario: SagaScenario) {
  ...
  countEvents(type: SagaEvent['type']): number {
    return this.events.filter((e) => e.type === type).length
  }

  firstEvent(type: SagaEvent['type']): SagaEvent | undefined {
