import type { Frame, ShardEvent, ShardLog, ShardScenario, ShardWorld } from './types.ts'
 ...
 * Снимок мира на каждом тике маленький — шарды с очередями и счётчики, — поэтому
...
export class Simulation {
  readonly scenario: ShardScenario
  readonly log: ShardLog

  private ctx: Ctx
  private frames: Frame[] = []

  constructor(scenario: ShardScenario) {
    this.scenario = scenario
    const world = createWorld(scenario)
    this.log = { series: [emptySample(world.x, world.shards.length)], done: [] }
    this.ctx = { world, events: [], scenario, log: this.log }
    this.frames.push({ world: structuredClone(world), events: [] })
  }

  get world(): ShardWorld {
  ...
  countEvents(type: ShardEvent['type']): number {
    return this.events.filter((e) => e.type === type).length
  }

  firstEvent(type: ShardEvent['type']): ShardEvent | undefined {
