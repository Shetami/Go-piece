...
import type { ShardEvent, ShardEventType, ShardScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: ShardScenario, opts: { autoPause: boolean }): Playback {
  ...
  const [pausedOn, setPausedOn] = useState<ShardEvent | null>(null)

  const seen = useRef(new Set<ShardEventType>())
