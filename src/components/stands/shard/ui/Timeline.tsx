...
import type { Frame, ShardEvent, ShardEventType } from '../engine/types.ts'
 ...
 * события шардов и решардирования: свой список важности и свой разбор.
 */

const markClass = (t: ShardEventType) => `mark mark-sh-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: ShardEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: ShardEvent[]
  selected: ShardEvent | null
  onSelect: (e: ShardEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-sh-${e.type.split('.')[0]}`} />
