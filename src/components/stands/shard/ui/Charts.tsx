...
import type { ShardLog } from '../engine/types.ts'
 ...
 * ось не прыгала. У каждого шарда свой цвет из категориальной палитры —
 * в том же порядке, что на карточках шардов. На остальных графиках синий —
 * обычная задержка, оранжевый — хвост и веерные запросы, бирюзовый — переезд.
 */
...
export interface ChartData {
  /** Запросов в шарде: очередь плюс работа; null — шарда ещё нет. */
  load: (number | null)[][]
  p50: (number | null)[]
  p99: (number | null)[]
  scatter: (number | null)[]
  anyScatter: boolean
  migrating: number[]
  anyMigrating: boolean
}

/** Всё, что рисуют графики, считается один раз на прогон. */
export function chartData(log: ShardLog): ChartData {
  const n = log.series.length
  const shards = Math.max(...log.series.map((s) => s.load.length))
  const byTick: number[][] = Array.from({ length: n }, () => [])
  const scatterByTick: number[][] = Array.from({ length: n }, () => [])
  for (const c of log.done) {
    byTick[c.t]?.push(c.lat)
    if (c.kind === 'scatter') scatterByTick[c.t]?.push(c.lat)
  }
  const win = (src: number[][], t: number) => src.slice(Math.max(0, t - LAT_WINDOW + 1), t + 1).flat()
  const p50: (number | null)[] = []
  const p99: (number | null)[] = []
  const scatter: (number | null)[] = []
  for (let t = 0; t < n; t++) {
    const all = win(byTick, t)
    p50.push(all.length ? percentile(all, 50) : null)
    p99.push(all.length ? percentile(all, 99) : null)
    const sc = win(scatterByTick, t)
    scatter.push(sc.length ? percentile(sc, 50) : null)
  }
  return {
    load: Array.from({ length: shards }, (_, r) => log.series.map((s) => s.load[r] ?? null)),
    p50,
    p99,
    scatter,
    anyScatter: log.done.some((c) => c.kind === 'scatter'),
    migrating: log.series.map((s) => s.migrating),
    anyMigrating: log.series.some((s) => s.migrating > 0),
  }
}
