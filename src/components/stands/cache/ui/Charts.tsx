...
import type { CacheLog } from '../engine/types.ts'
 ...
 * ось не прыгала. Цвет закреплён за сущностью: синий — попадания и обычная
 * задержка, оранжевый — промахи и хвост, бирюзовый — размер кэша,
 * красный — устаревшие ответы.
 */
...
export function chartData(log: CacheLog): ChartData {
  const n = log.series.length
  const byTick: number[][] = Array.from({ length: n }, () => [])
  for (const c of log.done) if (c.kind !== 'write') byTick[c.t]?.push(c.lat)
  ...
  return {
    hits: sum((i) => log.series[i]!.hits),
    misses: sum((i) => log.series[i]!.misses),
    stale: sum((i) => log.series[i]!.stale),
    anyStale: log.series.some((s) => s.stale > 0),
    p50,
    p99,
    dbBusy: log.series.map((s) => s.dbBusy),
    dbQueue: log.series.map((s) => s.dbQueue),
    cached: log.series.map((s) => s.cached),
  }
}
