...
import type { Frame, CacheEvent, CacheEventType } from '../engine/types.ts'
 ...
 * события кэша и базы: свой список важности и свой разбор.
 */

const markClass = (t: CacheEventType) => `mark mark-ca-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: CacheEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: CacheEvent[]
  selected: CacheEvent | null
  onSelect: (e: CacheEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-ca-${e.type.split('.')[0]}`} />
