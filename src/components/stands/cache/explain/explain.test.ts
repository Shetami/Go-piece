...
import { CACHE_SCENARIOS, coldStart, staleData, stampede } from '../engine/scenarios.ts'
import { EVENT_IMPORTANCE } from '../engine/types.ts'
import type { CacheScenario } from '../engine/types.ts'
...
const ALL: CacheScenario[] = [
  ...CACHE_SCENARIOS,
  { ...stampede, id: 'coalesce', config: { ...stampede.config, coalesce: true } },
  { ...stampede, id: 'swr', config: { ...stampede.config, staleWhileRevalidate: true, coalesce: true } },
  { ...staleData, id: 'delete', config: { ...staleData.config, invalidation: 'delete' } },
  { ...staleData, id: 'update', config: { ...staleData.config, invalidation: 'update' } },
  { ...coldStart, id: 'cold-coalesce', config: { ...coldStart.config, coalesce: true }, phases: [{ at: 100, x: 1.5 }] },
]
