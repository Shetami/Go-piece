import type { Frame, CacheEvent, CacheLog, CacheScenario, CacheWorld } from './types.ts'
 ...
 * Снимок мира на каждом тике маленький — кэш, база и счётчики, — поэтому
...
export class Simulation {
  readonly scenario: CacheScenario
  readonly log: CacheLog

  private ctx: Ctx
  private frames: Frame[] = []

  constructor(scenario: CacheScenario) {
  ...
  countEvents(type: CacheEvent['type']): number {
    return this.events.filter((e) => e.type === type).length
  }

  firstEvent(type: CacheEvent['type']): CacheEvent | undefined {
