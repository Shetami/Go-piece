...
import type { MemEvent, MemEventType, Snapshot } from '../engine/types.ts'
 ...
 * события аллокатора: свой список важности и свой разбор.
 */
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: MemEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: MemEvent[]
  selected: MemEvent | null
  onSelect: (e: MemEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
