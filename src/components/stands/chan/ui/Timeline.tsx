...
import type { ChanEvent, ChanEventType, Snapshot } from '../engine/types.ts'
 ...
 * события каналов: свой список важности и свой разбор.
 */
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: ChanEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: ChanEvent[]
  selected: ChanEvent | null
  onSelect: (e: ChanEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
