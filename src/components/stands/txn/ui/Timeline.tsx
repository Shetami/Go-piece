...
import type { Frame, TxnEvent, TxnEventType } from '../engine/types.ts'
 ...
 * события транзакций: свой список важности и свой разбор.
 */

const markClass = (t: TxnEventType) => `mark mark-db-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: TxnEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: TxnEvent[]
  selected: TxnEvent | null
  onSelect: (e: TxnEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-db-${e.type.split('.')[0]}`} />
