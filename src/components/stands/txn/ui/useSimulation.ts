...
import type { TxnEvent, TxnEventType, TxnScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: TxnScenario, opts: { autoPause: boolean }): Playback {
  ...
  const [pausedOn, setPausedOn] = useState<TxnEvent | null>(null)

  const seen = useRef(new Set<TxnEventType>())
