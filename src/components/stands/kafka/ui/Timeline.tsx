...
import type { KafkaEvent, KafkaEventType, Snapshot } from '../engine/types.ts'
import { EVENT_IMPORTANCE } from '../engine/types.ts'
import { explain } from '../explain/events.ts'

/**
 * Лента и перемотка. Устроены так же, как в соседних стендах, но знают про
 * события Kafka: свой список важности и свой разбор.
 */

const markClass = (t: KafkaEventType) => `mark mark-kf-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: KafkaEventType }[] = []
    for (const s of history) {
      const e = s.events.find((x) => EVENT_IMPORTANCE[x.type] === 'key') ?? s.events.find((x) => watchFor.includes(x.type))
  ...
  return (
    <div className="scrubber">
      <div className="scrubber-marks" aria-hidden="true">
        {marks.map((m) => (
          <span key={m.tick} className={markClass(m.type)} style={{ left: `${(m.tick / Math.max(1, last)) * 100}%` }} />
        ))}
      </div>
      <input
        type="range"
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: KafkaEvent[]
  selected: KafkaEvent | null
  onSelect: (e: KafkaEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-kf-${e.type.split('.')[0]}`} />
    ...
    <div className="detail" role="region" aria-label="Разбор события">
      ...
      {x.kafka && (
        <p className="detail-runtime">
          <span>В Kafka:</span> <code>{x.kafka}</code>
        </p>
      )}
      {x.model && (
