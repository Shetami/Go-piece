...
import type { KafkaEvent, KafkaEventType, KafkaScenario } from '../engine/types.ts'
...
export interface Playback {
  ...
  eventsUpTo: KafkaEvent[]
  counts: Partial<Record<KafkaEventType, number>>
  /** Событие, на котором автопауза остановилась в последний раз. */
  pausedOn: KafkaEvent | null
}
 ...
 */
export function useSimulation(scenario: KafkaScenario, seed: number, opts: { autoPause: boolean }): Playback {
  const sim = useMemo(
  ...
  const [pausedOn, setPausedOn] = useState<KafkaEvent | null>(null)

  const seen = useRef(new Set<KafkaEventType>())
  ...
  const counts = useMemo(() => {
    const c: Partial<Record<KafkaEventType, number>> = {}
