import type { Frame, ResilEvent, ResilLog, ResilScenario, ResilWorld } from './types.ts'
 ...
 * Снимок мира на каждом тике маленький — воркеры, очереди и счётчики, — поэтому
...
export class Simulation {
  readonly scenario: ResilScenario
  readonly log: ResilLog

  private ctx: Ctx
  private frames: Frame[] = []

  constructor(scenario: ResilScenario) {
  ...
  countEvents(type: ResilEvent['type']): number {
    return this.events.filter((e) => e.type === type).length
  }

  firstEvent(type: ResilEvent['type']): ResilEvent | undefined {
