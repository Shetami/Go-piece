import type { Fault, ResilConfig, ResilScenario, Phase } from '../engine/types.ts'
 ...
 * `s=cascade&timeout=8`. Фазы нагрузки едут в `p` как `40:2,60:1`,
...
const CONFIG_KEYS = Object.keys(resolveConfig(undefined)) as (keyof ResilConfig)[]
...
export function initialState(base: ResilScenario): StandState {
  return { base: base.id, config: resolveConfig(base.config), phases: null, faults: null }
}

export function toScenario(st: StandState): ResilScenario {
  const base = resilScenarioById(st.base)
...
export function decodeState(query: string, fallback: ResilScenario): StandState {
  const q = new URLSearchParams(query.replace(/^[#?]/, ''))
  const base = resilScenarioById(q.get('s') ?? '') ?? fallback
  ...
  st.config = cfg as unknown as ResilConfig
  const p = q.get('p')
  if (p !== null) {
    st.phases = p
      .split(',')
