...
import type { ResilLog } from '../engine/types.ts'
 ...
 * ось не прыгала. Цвет закреплён за сущностью: синий — своя работа и полные
 * ответы, оранжевый — ожидание зависимости и хвост, бирюзовый — заглушки,
 * красный — ошибки и отказы.
 */

export type SeriesColor = 'work' | 'wait' | 'in' | 'waste'
...
export interface ChartData {
  /** Воркеры A: заняты своей работой и ожиданием зависимости. */
  own: number[]
  waiting: number[]
  queue: number[]
  depQueue: number[]
...
export function chartData(log: ResilLog): ChartData {
  const n = log.series.length
  const all: number[][] = Array.from({ length: n }, () => [])
  const localOnly: number[][] = Array.from({ length: n }, () => [])
  for (const c of log.done) {
    if (c.kind === 'rejected') continue
    all[c.t]?.push(c.lat)
    if (!c.needsDep) localOnly[c.t]?.push(c.lat)
  }
  const win = (src: number[][], t: number) => src.slice(Math.max(0, t - LAT_WINDOW + 1), t + 1).flat()
  const p50: (number | null)[] = []
  const p99: (number | null)[] = []
  const localP99: (number | null)[] = []
  for (let t = 0; t < n; t++) {
    const w = win(all, t)
    p50.push(w.length ? percentile(w, 50) : null)
    p99.push(w.length ? percentile(w, 99) : null)
    const l = win(localOnly, t)
    localP99.push(l.length ? percentile(l, 99) : null)
  }
  ...
  return {
    own: log.series.map((s) => s.busy - s.waiting),
    waiting: log.series.map((s) => s.waiting),
    queue: log.series.map((s) => s.queue),
    depQueue: log.series.map((s) => s.depQueue),
    p50,
    p99,
    localP99,
    ok: sum((i) => log.series[i]!.ok),
    degraded: sum((i) => log.series[i]!.degraded),
    bad: sum((i) => log.series[i]!.errors + log.series[i]!.rejected),
    anyDegraded: log.series.some((s) => s.degraded > 0),
    anyBad: log.series.some((s) => s.errors + s.rejected > 0),
  }
}
