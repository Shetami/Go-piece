...
import type { ResilEvent, ResilEventType, ResilScenario } from '../engine/types.ts'
 ...
 */
export function useSimulation(scenario: ResilScenario, opts: { autoPause: boolean }): Playback {
  ...
  const [pausedOn, setPausedOn] = useState<ResilEvent | null>(null)

  const seen = useRef(new Set<ResilEventType>())
