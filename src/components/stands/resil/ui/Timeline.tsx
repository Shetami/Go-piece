...
import type { Frame, ResilEvent, ResilEventType } from '../engine/types.ts'
 ...
 * события сервиса и его зависимости: свой список важности и свой разбор.
 */

const markClass = (t: ResilEventType) => `mark mark-rs-${t.replace('.', '-')}`
...
export function Scrubber({ history, cursor, last, watchFor, onSeek }: {
  ...
  const marks = useMemo(() => {
    const out: { tick: number; type: ResilEventType }[] = []
...
export function EventFeed({ events, selected, onSelect, showMinor }: {
  events: ResilEvent[]
  selected: ResilEvent | null
  onSelect: (e: ResilEvent) => void
  showMinor: boolean
}) {
  const shown = useMemo(() => {
      ...
      {shown.map(({ e, key }) => (
        <li key={key}>
          ...
          >
            ...
            <span className={`feed-dot dot-rs-${e.type.split('.')[0]}`} />
