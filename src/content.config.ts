 ...
 *
 * У SQL-задач (тема из TASK_TOPICS с lang: 'sql') файлы другие:
 *
 *     schema.sql    таблицы и данные — заливаются в Postgres перед запросом
 *     starter.sql   запрос в редакторе при открытии
 *     solution.sql  эталонный запрос; его результат и есть правильный ответ
 *
...
const tasks = defineCollection({
  ...
  schema: z
    .object({
      ...
      /**
       * Только для SQL: важен ли порядок строк при сверке с эталоном. Без
       * ORDER BY в условии порядок не определён, и требовать его нечестно.
       */
